...
class FgModuleDeter(nn.Module):
    def __init__(self):
        nn.Module.__init__(self)

        self.Z_DIM = ARCH.Z_PRES_DIM + ARCH.Z_WHERE_DIM + ARCH.Z_DEPTH_DIM + ARCH.Z_WHAT_DIM
        self.T = ARCH.T[0]
        ...
        self.pres_depth_where_what_prior_prop = PresDepthWhereWhatPriorProp()
        self.pres_depth_where_what_post_prop = PresDepthWhereWhatPostProp()
        self.pres_depth_where_what_post_disc = PresDepthWhereWhatPostDisc()
        ...
        # Temporal rnn, used to encode history of z
    ...
    def get_dummy_things(self, B, device):
        ...
        return (h_post, c_post), (h_prior, c_prior), (z_pres, z_depth, z_where, z_what), ids
    ...
    def track(self, seq, bg, discovery_dropout):
        ...
        for t in range(T):
            ...
            things_t = dict(
                ...
                kl_what=kl[3],  # (B,)
                # For compatibility
                kl_dyna=torch.zeros_like(kl[3]),  # (B,)
                kl_fg=kl[0] + kl[1] + kl[2] + kl[3],  # (B,)
                ids=ids,  # (B, N)
                fg=fg,  # (B, C, H, W)
    ...
    def discover(self, x, z_prop, bg, start_id=0):
        ...
        (z_pres_post_prob, z_depth_post_loc, z_depth_post_scale, z_where_post_loc,
         z_where_post_scale, z_what_post_loc, z_what_post_scale) = self.pres_depth_where_what_post_disc(enc)
        ...
        z_depth_prior, z_where_prior, z_what_prior  = self.get_discovery_priors(x.device)
        # where prior, (B, G*G, 4)
    ...
    def propagate_gen(self, state_prev, z_prev, bg, sample=False):
        ...
        # Scale
        z_where[..., :2] = z_where_prev[..., :2] + ARCH.Z_SCALE_UPDATE_SCALE * torch.tanh(
            z_where_offset[..., :2])
        # Shift
        z_where[..., 2:] = z_where_prev[..., 2:] + ARCH.Z_SHIFT_UPDATE_SCALE * torch.tanh(
            z_where_offset[..., 2:])
        ...
        z_what = z_what_prev + ARCH.Z_WHAT_UPDATE_SCALE * torch.tanh(z_what_offset)
    ...
    def propagate(self, x, state_post_prev, state_prior_prev, z_prev, bg):
        ...
        if N == 0:
            ...
            return state_post_prev, state_prior_prev, z_prev, (0.0, 0.0, 0.0, 0.0), z_prev[2]
        ...
        (z_pres_prob, z_depth_offset_loc, z_depth_offset_scale, z_where_offset_loc,
         z_where_offset_scale, z_what_offset_loc, z_what_offset_scale) = self.pres_depth_where_what_post_prop(enc)
        ...
        # Other priors
        (z_pres_prob, z_depth_offset_loc, z_depth_offset_scale, z_where_offset_loc,
         z_where_offset_scale, z_what_offset_loc, z_what_offset_scale) = self.pres_depth_where_what_prior_prop(h_prior)
    ...
    def compute_prop_map(self, z_prop):
        """
        Compute a feature volume for discovery conditoining
        Args:
            z_prop:
                z_pres_prop: (B, N, D)
                z_depth_prop: (B, N, D)
        ...
        z_pres_prop, z_depth_prop, z_where_prop, z_what_prop = z_prop
    ...
    def compute_prop_cond(self, z, state_prev, prior_or_post):
        """
        Use a graph network to compute conditions for propagation
        Args:
            z: z_t
                z_pres: (B, N, 1)
                z_depth: (B, N, 1)
    ...
    def rejection(self, z_disc, z_prop, threshold):
        ...
        Returns:
            z_disc
        """
        z_pres_disc, z_depth_disc, z_where_disc, z_what_disc  = z_disc
        z_pres_prop, z_depth_prop, z_where_prop, z_what_prop  = z_prop
        # (B, N1, N2, 1)
        ...
        return (z_pres_disc_new, z_depth_disc, z_where_disc, z_what_disc)
...
class PresDepthWhereWhatPostProp(nn.Module):
    """Predict posterior given proposals in propagation"""
    
    def __init__(self):
        nn.Module.__init__(self)
        self.enc = MLP(
            [ARCH.RNN_HIDDEN_DIM + ARCH.PROPOSAL_ENC_DIM, 128, 128,
    ...
    def forward(self, enc):
        ...
        return z_pres_prob, z_depth_loc, z_depth_scale, z_where_loc, z_where_scale, z_what_loc, z_what_scale


class PresDepthWhereWhatPriorProp(nn.Module):
    """Predict posterior given proposals in propagation"""
...
class ProposalEncoder(nn.Module):
    """Same as glimpse encoder, but for encoding proposal"""
...
class PresDepthWhereWhatPostDisc(nn.Module):
    """Predict posterior given image encoding and propagation map in discovery"""
    ...
    def forward(self, x):
        ...
        (z_pres_prob, z_depth_loc, z_depth_scale, z_where_loc, z_where_scale,
         z_what_loc, z_what_scale) = torch.split(params,
                                                                               [ARCH.Z_PRES_DIM] + [
                                                                                   ARCH.Z_DEPTH_DIM] * 2 + [
                                                                                   ARCH.Z_WHERE_DIM] * 2 + [
                                                                                   ARCH.Z_WHAT_DIM] * 2, dim=-1)
        z_pres_prob = torch.sigmoid(z_pres_prob)
        z_where_scale = F.softplus(z_where_scale) + 1e-4
        z_depth_scale = F.softplus(z_depth_scale) + 1e-4
        z_what_scale = F.softplus(z_what_scale) + 1e-4
